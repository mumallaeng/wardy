...
test("M-01과 M-05 배포 artifact 및 M-05 dataset은 고정된 Hugging Face revision을 사용한다", async () => {
  ...
  assert.match(m01.revision, /^[a-f0-9]{40}$/);
  ...
  assert.match(m05.revision, /^[a-f0-9]{40}$/);
  ...
  const envDefault = (name, target) => {
    const escapedTarget = target.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    return new RegExp(
      "set_env_default\\s+\\\\?\\s*" + name +
      "\\s+\\\\?\\s*\\\"" + escapedTarget + "\\\"",
    );
  };
  assert.match(setup, envDefault(
    "WARDY_PERSON_ENGINE",
    "edge/models/m01_person/" + m01Model.default_version + "/model.engine",
  ));
  assert.match(setup, envDefault(
    "WARDY_HAZARD_MODEL",
    "edge/models/m05_hazard/" + m05Model.default_version + "/model.onnx",
  ));
...
test("AI와 camera 상태 갱신은 직렬화되고 낙상 event는 전이 시에만 적용된다", async () => {
  const api = await readFile(path.join(root, "edge/src/api/mjpeg_service.cpp"), "utf8");
  const eventRuntime = await readFile(
    path.join(root, "edge/src/rules/event_runtime.cpp"), "utf8",
  );
...
test("내부 추론 예외는 웹 상태에 그대로 노출하지 않는다", async () => {
  const api = await readFile(path.join(root, "edge/src/api/mjpeg_service.cpp"), "utf8");
  const personRuntime = await readFile(
    path.join(root, "edge/src/inference/person_inference_runtime.cpp"), "utf8",
  );
...
test("Jetson 일일 요약은 고정한 로컬 Nemotron 모델과 안전 fallback을 사용한다", async () => {
  ...
  assert.match(header, /model = "nemotron-3-nano:4b"/);
  assert.match(versions, /^WARDY_OLLAMA_VERSION=0\.32\.5$/m);
  assert.match(versions, /^WARDY_OLLAMA_INSTALL_SHA256=[a-f0-9]{64}$/m);
  assert.match(versions, /^WARDY_LLM_MODEL=nemotron-3-nano:4b$/m);
  assert.match(
    versions,
    /^WARDY_LLM_MODEL_DIGEST=6cc467f054393a55e98a74098abde0c762ffb6d1d8cd64becf30458f38886197$/m,
  );
  assert.match(installer, /sha256sum --check --status/);
  ...
  assert.match(source, /kMaximumPromptEvents = 12/);
  assert.match(source, /관찰 기록을 요약하거나 답으로 쓰지 마라/);
